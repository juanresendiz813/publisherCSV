...
> This is the README of the upstream project this repo is derived from,
> [malloydata/publisher](https://github.com/malloydata/publisher), preserved verbatim from commit
> `e318cae1` (0.4.0) — only its relative links were rewritten to resolve from `docs/`. Our own
> project README is [`../README.md`](../README.md).
...
<p align="center">
  <a href="https://github.com/user-attachments/assets/376a809d-8016-41a7-9464-a5634ea0589d"><img src="malloy-publisher-demo.gif" alt="Malloy Publisher serving the bundled storefront dashboard" width="800"></a>
</p>
...
layout, the bare `npx` form — are in [docs/scaffolding.md](scaffolding.md).
...
[connection](connections.md) — BigQuery, Snowflake, Postgres, Databricks, MotherDuck, and more —
...
[docs/configuration.md](configuration.md#semantic-ranking-for-search_database_schema).
...
[docs/configuration.md](configuration.md#the-mcpjson-the-server-writes).
...
[docs/ai-agents.md](ai-agents.md). An agent working unattended that started the server itself uses
...
- **Ask in plain English.** An agent grounds itself with `get_context`, runs
  ...
  pitfalls and how to write up a finding — [docs/ai-agents.md](ai-agents.md).
- **Work in notebooks.** `.malloynb` notebooks live inside a package, mix prose and queries, and run on
  the same governed endpoints — [docs/choosing-a-surface.md](choosing-a-surface.md).
- **Explore, no code.** Build and drill into queries visually with [Malloy Explorer](explorer.md);
...
- **Dashboards declared in Malloy.** A `dashboards/*.malloy` file _is_ the dashboard: filterable,
  ...
  reviews like any other source file — [docs/dashboards.md](dashboards.md).
- **No-build HTML data apps.** Ship HTML, CSS, and JavaScript inside a package and Publisher hosts it
  against the model — [docs/html-data-apps.md](html-data-apps.md).
- **The Publisher Console.** Browse packages, models, and every artifact in the built-in web UI, with
  your own [colors, fonts, and dark mode](theming.md) — [docs/console.md](console.md).
...
- **Decide who sees what.** [Givens](givens.md) declare runtime parameters and drive filter
  widgets; [row-level access](row-level-access.md) and [`#(authorize)`](authorize.md) gate
  which rows a caller gets and whether they may query a source at all.
- **Decide what is visible.** Curate what is [discoverable and queryable](discovery-and-access.md)
  separately, so an agent sees only the sources you meant it to.
- **Know the boundary.** [docs/security-posture.md](security-posture.md) lists what Publisher
...
- **Pre-aggregate.** `#@ preaggregate` rolls a measure up to a coarse grain so covered queries read a
  small table instead of the fact table — [docs/preaggregation.md](preaggregation.md).
- **Store it where you like.** Persist into a [DuckLake](ducklake.md) storage tier, attach a
  DuckLake catalog read-only, and run offline or air-gapped —
  [docs/persist-storage-tutorial.md](persist-storage-tutorial.md).
- **Attribute every query.** [Query metadata](query-metadata.md) tags each statement with a team,
...
- **Any data.** DuckDB is built in for CSV, Parquet, JSON, and Excel; connect BigQuery, Snowflake,
  Postgres, MySQL, Trino, Databricks, MotherDuck, and DuckLake — [docs/connections.md](connections.md).
- **Any host.** `npx`, Docker, or Docker Compose, in minutes — [docs/deployment.md](deployment.md).
- **Alongside dbt.** Where Malloy and dbt fit together, and the plan to close the gaps —
  [docs/dbt-roadmap.md](dbt-roadmap.md).
...
The [`docs/`](./) folder is the reference hub; start at its [index](README.md). Beyond the
guides linked above: [architecture](architecture.md) for how the pieces fit together,
[api-overview](api-overview.md) for the REST and MCP surface, [packages](packages.md) for
...
Build and hack on Publisher from a clone with [docs/development.md](development.md); contribution
